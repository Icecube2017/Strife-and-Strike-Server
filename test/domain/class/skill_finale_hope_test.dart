...
void main() {
  group('SkillFinaleHope', () {
    test('can rewrite a revealed attack dice result during the response window', () async {
      ...
      expect(
        (state.decision!.payload['diceRoll'] as Map<String, dynamic>)['finalResult'],
        2,
      );

      await engine.processAction(beta, ActionType.skill, {
        'skillId': SkillId.finaleHope.id,
    ...
    test('can rewrite a revealed trait dice result during the response window', () async {
      final alpha = _FixedDiceCharacter(
        'char_alpha',
        TemplateId.defensive.id,
        RaceId.human.id,
