...
void main() {
  group('GameEngine', () {
    ...
    test(
      'response action can replace the target of a previous action',
      () async {
        final alpha = _TestCharacter('char_alpha');
        final beta = _TestCharacter('char_beta');
    ...
    test(
      'getActionBlockReason validates attackCard multi-card payload structure',
      () async {
        final alpha = _TestCharacter('char_alpha');
        final beta = _TestCharacter('char_beta');
        final state = GameState(
    ...
    test(
      'getActionBlockReason validates limitedCard multi-card payload structure',
      () async {
        final alpha = _TestCharacter('char_alpha');
        final beta = _TestCharacter('char_beta');
        final state = GameState(
    ...
    test(
      'getActionBlockReason rejects mismatched play-card action type',
      () async {
        final alpha = _TestCharacter('char_alpha');
        final beta = _TestCharacter('char_beta');
        alpha.hand.add(
          _EngineTestPropCard('limited_card', isAttackLimited: true),
        );
        final state = GameState(
          [
            Player('player_alpha', 'Alpha', [alpha], 0),
    ...
    test(
      'attackCard writes dice result into the response window payload',
      () async {
        ...
        expect(state.pendingStack, hasLength(1));
        expect(
          state.pendingStack.single.stage,
          PendingActionStage.waitingResponse,
        );
        expect(state.pendingStack.single.resolutionState.diceRoll, isNotNull);
        expect(
          state.pendingStack.single.resolutionState.diceRoll!.finalResult,
          2,
        );
        expect(state.pendingStack.single.resolutionState.baseDamage, isNull);
        expect(state.pendingStack.single.resolutionState.pendingDamage, isNull);
        expect(alpha.hand, [card]);
        expect(state.decision, isNotNull);
        expect(
          state.decision!.payload['actionId'],
          state.pendingStack.single.actionId,
        );
        expect(
          state.decision!.payload['stage'],
          PendingActionStage.waitingResponse.name,
        );
        ...
        expect(state.decision!.payload.containsKey('baseDamage'), isFalse);
        expect(state.decision!.payload.containsKey('pendingDamage'), isFalse);

        await engine.passPriority('player_beta');

        expect(alpha.hand, isEmpty);
        expect(beta.currentHp, 14);
      },
    );
  });
}
